...
public:
	...
	virtual ~Rpd() = default;
	virtual void draw(const vector<Mat>& designImages, const vector<vector<Tooth>>& teeth) const = 0;
};
...
public:
	...
	void draw(const vector<Mat>& designImages, const vector<vector<Tooth>>& teeth) const override;
};
